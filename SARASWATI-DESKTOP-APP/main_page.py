# -*- coding: utf-8 -*-
"""
Created on Sun Apr 21 20:16:48 2024
...
import customtkinter as ctk  # assuming customtkinter is used for styling
import pandas as pd
import threading
...
def create_main_page(parent):
    ...
    loaded_df = None  # Local variable, no global declaration needed

    def load_file():
        nonlocal loaded_df  # Correct scope definition
        filepath = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")])
        if filepath:
            file_path_label.configure(text=filepath)
            loaded_df = pd.read_csv(filepath)
            update_comboboxes(loaded_df.columns)
    ...
    # Rest of your UI setup code here, e.g., buttons, labels, etc.
    ...
    status_label = ctk.CTkLabel(frame, text="")  # Parent is 'frame', not 'app'
